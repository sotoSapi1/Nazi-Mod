...
std::string dump_method(Il2CppClass *klass) {
    ...
    while (auto method = il2cpp_class_get_methods(klass, &iter)) {
        ...
        if (method->methodPointer) {
            ...
            outPut << "\t// SignatureCondition: " << GetMethodSignaturePattern(method) << "\n";
            outPut << "\t// Index: 0x" ;
            outPut << std::hex << index;
            outPut<<  " Offset: 0x" << std::hex << (uint64_t) method->methodPointer - il2cpp_base;
            outPut << " VA: 0x";
...
std::string dump_type(const Il2CppType *type) {
    ...
    outPut << "\n{\n\n";
    outPut << dump_field_pattern(klass);
